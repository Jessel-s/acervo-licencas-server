...
// Serial sequencial automatico (PDV-001, PDV-002, ...).
// Nao precisa ser preenchido pelo admin.
async function gerarSerialPdv(): Promise<string> {
  const { count } = await supabase
    .from("licencas")
    .select("id", { count: "exact", head: true });
  const base = (count ?? 0) + 1;
  // Tenta alguns numeros a frente em caso de corrida (dois cadastros simultaneos)
  for (let i = 0; i < 25; i++) {
    const candidato = `PDV-${String(base + i).padStart(3, "0")}`;
    const { data } = await supabase
      .from("licencas")
      .select("id")
      .eq("serial_pdv", candidato)
      .maybeSingle();
    if (!data) return candidato;
  }
  // Fallback: nunca deve acontecer, mas garante que o cadastro nao quebre
...
async function novaLicenca(user: { id: string; email?: string }, colegioId: string) {
  ...
  if (error) throw error;
  await registrarAuditoria(user.id, user.email, colegioId, "nova_licenca", { serial_pdv: serialPdv });
...
async function alterarStatus(user: { id: string; email?: string }, colegioId: string, novoStatus: string) {
  ...
  await supabase
    ...
    .eq("colegio_id", colegioId);
  await registrarAuditoria(user.id, user.email, colegioId, novoStatus === "ativo" ? "reativar" : "suspender", {});
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      // 'cadastrar' NAO exige colegio_id (o cliente ainda nao existe)
      if (acao !== "cadastrar" && !colegioId) {
        return json({ ok: false, mensagem: "colegio_id obrigatorio" }, 400);
      }

      if (acao === "cadastrar") {
        return await cadastrarCliente(user, body);
      }
      if (acao === "renovar") {
        const dias = Number.isInteger(body?.dias_validade) && body.dias_validade > 0
          ? Math.min(body.dias_validade, 3650)
          : 365;
        return await renovarCliente(user, colegioId, dias);
      }
      if (acao === "definir_expiracao") {
        ...
        return await definirExpiracao(user, colegioId, data);
      }
      if (acao === "suspender" || acao === "reativar") {
        return await alterarStatus(user, colegioId, acao === "reativar" ? "ativo" : "suspenso");
      }
      if (acao === "editar") {
      ...
      if (acao === "nova_licenca") {
        return await novaLicenca(user, colegioId);
      }
    ...
    console.error("Falha ao gerenciar clientes:", msg);
    return json({ ok: false, mensagem: `Nao foi possivel concluir a operacao: ${msg}` }, 500);
  }
});
